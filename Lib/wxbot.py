...
class WXBot:
    ...
    def get_contact(self):
        ...
        r.encoding = 'utf-8'
        if self.DEBUG:
            with open(os.path.join(self.temp_pwd,'contacts_'+self.uuid+'.json'), 'w') as f:
        ...
        if self.DEBUG:
            with open(os.path.join(self.temp_pwd,'contact_list_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.contact_list))
            with open(os.path.join(self.temp_pwd,'special_list_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.special_list))
            with open(os.path.join(self.temp_pwd,'group_list_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.group_list))
            with open(os.path.join(self.temp_pwd,'public_list_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.public_list))
            with open(os.path.join(self.temp_pwd,'member_list_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.member_list))
            with open(os.path.join(self.temp_pwd,'group_users_'+self.uuid+'.json'), 'w') as f:
                f.write(json.dumps(self.group_members))
            with open(os.path.join(self.temp_pwd,'account_info_'+self.uuid+'.json'), 'w') as f:
    ...
    def run(self):
        try:
            ...
            if result != SUCCESS:
                ...
                #该二维码已失效，将其删除
                os.remove('static/temp/'+str(self.uuid)+'.png')
                return
